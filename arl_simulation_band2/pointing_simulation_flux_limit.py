...
original_components = create_test_skycomponents_from_s3(flux_limit=0.0001,
                                                        phasecentre=phasecentre,
                                                        polarisation_frame=PolarisationFrame("stokesI"),
                                                        frequency=numpy.array(frequency),
                                                        radius=pbradius * HWHM)

for flux_limit in [0.1, 0.03, 0.01, 0.003, 0.001, 0.0003, 0.0001]:
    result = dict()
    result['ra'] = ra
    result['dec'] = dec
    ...
    print("RA %.1f Dec %.1f number components %d peak flux %.3f total flux %.3f" % (ra, dec,
                                                                                    len(filtered_components),
                                                                                    max_flux, total_flux))
    filtered_components = [copy_skycomponent(c) for c in filtered_components]
    plt.clf()
    show_image(pb, components=filtered_components)
    plt.savefig('component_image_flux_limit_%.3f_dec_%.1f.png' % (flux_limit, dec))
...
flux_limits = [r['flux_limit'] for r in results]
plt.semilogx(flux_limits, mflux, '.', label='Max flux')
plt.semilogx(flux_limits, tflux, '.', label='Total flux')
plt.xlabel('Flux limit (Jy)')
plt.ylabel('Flux')
plt.legend()
plt.savefig('fluxes_vs_flux_limit.png')
plt.show(block=False)
...
flux_limits = [r['flux_limit'] for r in results]
plt.loglog(flux_limits, nbcomps, '.', label='Number before PB')
plt.loglog(flux_limits, ncomps, '.', label='Number after PB')
plt.xlabel('Flux limit (Jy)')
plt.ylabel('Number components')
plt.legend()
plt.savefig('number_components_vs_flux_limit.png')
plt.show(block=False)
