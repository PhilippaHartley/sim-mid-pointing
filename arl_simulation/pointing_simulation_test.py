...
#    export_image_to_fits(dirty, 'no_PE_with_noise.fits')
   ...
   # export_image_to_fits(dirty, 'residuals_no_PE_with_noise.fits')
    ...
    meq_PE =numpy.load('meqtrees_dlm2.npy').T
    print ('meqpe shape',meq_PE.shape)
    print (meq_PE)
    print (numpy.ravel(meq_PE ))
    meq_PE = numpy.reshape(meq_PE, (65,197,1,1,2))
    print ('reshaped', meq_PE)
    ...
    for pe in pes:
        ...
        print("Pointing errors: global (%.1f, %.1f) arcsec, static %.1f arcsec, dynamic %.1f arcsec" %
              (global_pointing_error[0], global_pointing_error[1], static_pointing_error,
               pointing_error))
        print('pointing_error', pointing_error)
        error_pt = simulate_pointingtable(pt, pointing_error=pointing_error * a2r,
                                          ...
                                          seed=seed)
        print('arlerror_pt' , error_pt.data['pointing'])#error_pt.__dict__['data'][0][0].shape)
        #export_pointingtable_to_hdf5(error_pt,
                                    # 'pointingsim_%s_error_%.0farcsec_pointingtable.hdf5' % (context, pe))
        error_pt.data['pointing'] = meq_PE
        print('arlerror_pt' , error_pt.data['pointing'])#error_pt.__dict__['data'][0][0].shape)
