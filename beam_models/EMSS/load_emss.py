...
for k , pol in enumerate(pols):
    for i in range(beamsize):
        for j in range(np.int(beamsize/2)):
            azel[k,j,i]= ((poldict[pol][(np.abs(r[i,j]-th).argmin(),np.abs(theta[i,j]-ph_rad).argmin())]))
...
fig, axes = plt.subplots(nrows=2, ncols=2, sharex = True, sharey = True, figsize = (4,6))
for count, ax in enumerate(axes.flat):
    amp_azel = np.absolute(azel)
    # db = 10*np.log((amp_azel)/np.max(amp_azel)) # power dB not field
    im = ax.imshow(np.angle(poldict[pols[count]]), vmin = -np.pi, vmax = np.pi, origin = 'lower', extent = [0,180, 0, 6], aspect = (30*501)/73)
...
amp_azel_JVH = np.absolute(azel[2,:,:])
db_JHH = 10*np.log((amp_azel_JVH)/np.max(amp_azel_JVH))
...
for count, ax in enumerate(axes.flat):
    amp_azel = np.absolute(azel)
    db = 10*np.log((amp_azel)/np.max(amp_azel)) # power not voltage
    im = ax.imshow((db[count,:,:]), vmin = -70, vmax = 0, origin = 'lower', extent = [-6, 6, -6, 6])
    if (count==2) or (count==3):
...
plt.tight_layout()
fig.colorbar(im, ax=axes.ravel().tolist(), use_gridspec=True)
plt.savefig('amplitudes_azel.png')
