...
def make_config_file(lerrs, merrs,ms_name, source_list_name):
   ...
   #     merrs =     ' '.join(map(str,merrs))
    print 'l and m sigma', lerrs, merrs
    ...
    f.write('oms_pointing_errors.pe_l.error_model = RandDynamic\n')
    f.write('oms_pointing_errors.pe_l.sigma = %s\n'%lerrs)
    f.write('oms_pointing_errors.pe_m.error_model = RandDynamic\n')
    f.write('oms_pointing_errors.pe_m.sigma = %s\n'%merrs)
